...
- `04-deploy-lambda.yml` — Lesson 04: `test → build-and-smoke-test (docker build + RIE invoke with exact-value asserts) → deploy (main only, skipped until AWS variables exist; OIDC assume-role, ECR push by git SHA, lambda update-function-code, remote smoke test)`.
- `05-progressive-deploy.yml` — Lesson 05: `test → progressive-deploy`: train blue+green, fleet up, shadow replay, promotion gate (red gate = rollback by abstention; report uploads `if: always()`), canary with the deterministic 2-of-10 assertion, full switch asserting green's exact probability.
